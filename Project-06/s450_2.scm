...
; (define (eval-assignment exp env)
  ; (let ((name (assignment-variable exp)))
    ; (cond 
		; ((lookup-action name)
			; (error "CANT CHANGE VARIABLE: " name))
	; (else 
...
; (define (eval-definition exp env)
  ; (let ((name (definition-variable exp)))  
    ; (cond 
		; ((lookup-action name)
			; (error "CANT CHANGE DEFINITION: " name))
	; (else
...
(define (install-primitive-procedure name action)
  (cond 
	((lookup-action name)  ;; special form check
	  (error "Wrong name: " name))
  (else 
	(begin
...
; (define (make-unbound! exp env)
  ; (define (scan bindings)
    ; (cond ((null? bindings)
           ; (error "Unbound variable -- MAKE-UNBOUND!" exp))
          ; ((equal? exp (car (car bindings)))
           ; (set! bindings (cdr bindings)))
          ; (else (scan (cdr bindings)))))
  ; (scan (first-frame env)))
 ; (locally-make-unbound! 
; )
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; 
;;hw6 added install-special form
(install-special-form 'load eval-load)
(define (install-primitive-procedure 'car car)
(define (install-primitive-procedure 'cdr cdr)
(define (install-primitive-procedure 'cons cons)
(define (install-primitive-procedure 'null? null?)
(define (install-primitive-procedure 'assoc assoc)
(define (install-primitive-procedure '+ +)
(define (install-primitive-procedure '- -)
(define (install-primitive-procedure '* *)
(define (install-primitive-procedure '/ /)
(define (install-primitive-procedure '> >)
(define (install-primitive-procedure '< <)
(define (install-primitive-procedure '= =)
(define (install-primitive-procedure 'display display)
(define (install-primitive-procedure 'newline newline)
(define (install-primitive-procedure 'member member)
(define (install-primitive-procedure 'zero? zero?)
(install-special-form 'load eval-load)
