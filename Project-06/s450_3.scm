...
(define	(frames)(first-frame env))
			
(define	(var-frames)(frame-variables frames)) 
				
(define	(val-frames)(frame-values frames))

(define (scanning pre-vars pre-vals var-frames val-frames) 
       (if (not (null? var-frames)) 
           (cond ((equal? exp (car var-frames)) 
             (begin (set-car! frames (cdr var-frames)) 
                    (set-cdr! frames (cdr val-frames)))) 
             (else (scanning 
			 var-frames val-frames 
			 (cdr var-frames) (cdr val-frames)))
		)
	))
...
(define (locally-defined? exp env)
  (cond ((member (car (cdr exp)) (frame-variables (first-frame env)))
      #t)
      (else #f)
	  ))
...
(define (make-unbound! exp env)    
     (if (not (null? var-frames)) 
         (cond ((equal? exp (car var-frames)) 
             (begin (set-car! frames (cdr var-frames)) 
                    (set-cdr! frames (cdr val-frames)))) 
             (else (scanning var-frames val-frames 
				(cdr var-frames) (cdr val-frames))
		))
	)
) 
...
(define (locally-make-unbound! exp env)
      ...
      (set-car! frames (remove-val var-names var-frames))
      (set-cdr! frames (scanning
       var-frames val-frames var-names))
      (car (cdr exp))
  )
