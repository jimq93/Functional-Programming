...
(define (procedure-basic-parameters p)
  (define (params-to-basic params)
    (if (null? params) '()
        (cons
           (cond ((or (delayed? (car params)) 
						(dynamic? (car params))
						(reference? (car params)))
                 (thunk-exp (car params)))
                 (else (car params)))
...
(define (cons-stream exp env)
  (cons (cadr exp) (make-thunk (car (cdr (cdr exp))) env)))

(define (stream-car strm) (car strm))
(define (stream-cdr strm) (force-thunk (cdr strm)))
(define (stream-null? strm) (equal? strm the-empty-environment))
...
(define (reference-env ref) (car (cdr (cdr ref))))
(define (reference-exp ref-obj)(car (cdr ref-obj)))
