    ...
    (let ((pc (make-register))
      ...
      (let ((the-ops
            ...
            (cond ((null? insts)
				...
				;; check for a breakpoint before actually executing. only the
                  ...
                  (let ((current-breakpoint
                    ...
                    (cond ((null? current-breakpoint)
                        (actually-execute))
						(else (let ((breakpoint (car current-breakpoint)))
                          (display* "Breakpoint: "
                                    (car breakpoint)
                                    ", offset "
                                    (cadr breakpoint))
        ...
        (define (actually-execute)
          (cond ((null? (get-contents pc))
              'done)
              (else (begin
                ((cadr (car (get-contents pc))))
                (execute))))) 
        ...
        (define (dispatch message)
          (cond ((equal? message 'start)
			   ...
			   ;;HW8 Added 
				...
				((equal? message 'install-labels)
                 (lambda (labs) (set! labels labs)))
                
                ((equal? message 'add-breakpoint)
                 (lambda (label n)
                   (set! breakpoints (cons (create-breakpoint label n labels)
                                           breakpoints))))
                ((equal? message 'remove-breakpoint)
                 (lambda (label n)
                   (let ((bp (create-breakpoint label n labels)))
                     (if (member bp breakpoints)
                         (set! breakpoints (val-remove bp breakpoints))))))
                 ((equal? message 'remove-all-breakpoints)
		...
		(else (extract-labels-count
		 ...
		 (lambda (insts labels) 
             (cond ((symbol? (car text))
				 ...
				 (else (receive insts
                          (cons (make-entry-label (car text)
                                                  insts
                                                  count)
                                labels))))
					)
             (else (receive (cons (make-instruction (car text) count)
                                insts) labels)
				)
			)
			)
...
; (define (make-entry-label label-name insts)
...
; (define (lookup-label labels label-name)
  ; (let ((val (assoc label-name labels)))
	...
	;;HW8 Added 
	(define (make-entry-label label-name insts counter)
  (list label-name insts counter))

(define (entry-label-name entry) (car entry))
(define (entry-label-insts entry) (cadr entry))
(define (entry-label-counter entry) (caddr entry))

(define (lookup-label labels label-name)
  (let ((val (assoc label-name labels)))
    (if val
        (entry-label-insts val)
        (error "Undefined label -- ASSEMBLE " label-name))))
...
(define (create-breakpoint label n labels)
  (let ((entry-label (assoc label labels)))
    (if entry-label
        (list label n (+ n (caddr entry-label)))
        (error "No Label, Create Breakpoint Here" label))))
