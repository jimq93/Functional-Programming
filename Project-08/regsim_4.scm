      ...
      (let ((the-ops
		...
		(cond ((not (null? sequence)) 
			(cond ((instruction (car sequence)) 
				...
				(else (filter-breakpoint (cdr sequence) instruction))
			))
		(else '())))  
		(define (breakpoint-remove value this)
		(filter-breakpoint this (lambda (n) (not (equal? n value)))))
          ...
          (cond ((equal? message 'start)
			   ...
			   ;;HW8 Added   
                ((equal? message 'set-breakpoint)
                 (lambda (label n)
                   (set! breakpoints 
				   (cons (breakpoint-here label n labels)
                       breakpoints))))
                ((equal? message 'cancel-breakpoint)
                 (lambda (label n)
                   (let ((breakpt (breakpoint-here label n labels)))
                     (if (member breakpt breakpoints)
                         (set! breakpoints 
						 (breakpoint-remove breakpt breakpoints))))))
...
(define (breakpoint-here label n labels)
    (cond ((assoc label labels)
        (list label n (+ n (caddr (assoc label labels)))))
