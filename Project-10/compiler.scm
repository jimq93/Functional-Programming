...
(define (compile-if exp target linkage)
    (let ((t-branch (make-label 'true-branch))
      ...
      (let ((consequent-linkage
             (if (equal? linkage 'next) after-if linkage)))
...
(define (compile-lambda exp target linkage)
  (let ((proc-entry (make-label 'entry))
    ...
    (let ((lambda-linkage
           (if (equal? linkage 'next) after-lambda linkage)))
      (append-instruction-sequences
...
(define (compile-procedure-call target linkage)
  (let ((primitive-branch (make-label 'primitive-branch))
    ...
    (let ((compiled-linkage
           (if (equal? linkage 'next) after-call linkage)))
      (append-instruction-sequences
...
(define (compile-proc-appl target linkage)
    (cond ((and (equal? target 'val) (not (equal? linkage 'return)))    ;;; CASE 1
           (make-instruction-sequence
            ...
            `((assign continue (label ,linkage))
              ...
              (goto (reg val)))))
          ;; ---------------------------------------------------------------
          ((and (not (equal? target 'val))                           ;;; CASE 2
                (not (equal? linkage 'return)))
           (let ((proc-return (make-label 'proc-return)))
             (make-instruction-sequence
              ...
              `((assign continue (label ,proc-return))
                ...
                (goto (label ,linkage))))))
          ;; ---------------------------------------------------------------
          ((and (equal? target 'val) (equal? linkage 'return))          ;;; CASE 3
           (make-instruction-sequence
            ...
            '((assign val (op compiled-procedure-entry)
              ...
              (goto (reg val)))))
          ;; ---------------------------------------------------------------
          ((and (not (equal? target 'val)) (equal? linkage 'return))    ;;; CASE 4
...
;;HW10 Part 2 Added APPLY

(define (compile-apply exp target linkage)
	(letrec
		((proc-code (compile 
			(car (cdr exp)) 'proc 'next))
		(operand-codes (compile 
			(car (cddr exp)) 'argl 'next)))
	(preserving '(env continue)
		proc-code
		(preserving '(proc continue)
			(construct-arglist operand-codes)
