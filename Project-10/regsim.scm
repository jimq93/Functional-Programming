  ...
  (let ((s '())
    ...
    (define (dispatch message)
      (cond ((equal? message 'push) push)
            ((equal? message 'pop) (pop))
            ((equal? message 'initialize) (initialize))
            ((equal? message 'print-statistics)
             (print-statistics))
            (else
...
(define (make-new-machine)
    (let ((pc (make-register))
      ...
      (let ((the-ops
			...
			(cond ((not (null? insts)) 
                  (let ((current-breakpoint
                         (filter-breakpoint
                          breakpoints
                          (lambda (breakpoint)
                            (equal? (caddr breakpoint)
                               (caddar insts))))))
                    (cond ((not (null? current-breakpoint))
                        (let ((breakpoint (car current-breakpoint)))
                          (display* 
						  "Breakpoint: " (car breakpoint)
                                ", at " (cadr breakpoint))
                          (newline)))
					(else (proceed-now)))))
                (else 'done))))
		;;HW8 5.19 Added
        (define (proceed-now)
          (cond ((not (null? (get-contents pc)))
              (begin
                ((cadar (get-contents pc)))
                (execute)))
              (else 
			  'done))) 
		
		(define (filter-breakpoint sequence instruction)
		(cond ((not (null? sequence)) 
			(cond ((instruction (car sequence)) 
				(cons (car sequence) 
					(filter-breakpoint (cdr sequence) instruction)))
				(else 
					(filter-breakpoint (cdr sequence) instruction))))
		(else '())))  
		
		(define (breakpoint-remove breakpoint this)
		(filter-breakpoint this (lambda (n) (not (equal? n breakpoint)))))
		
		(define (breakpoint-here label at labels)
			(cond ((assoc label labels)
				(list label at (+ at (caddr (assoc label labels)))))
			(else (error "No Label, Create Breakpoint Here" label))))
        
		(define (dispatch message)
          (cond ((equal? message 'start)
			   ...
			   ;;HW8 5.19 Added   
                ((equal? message 'set-breakpoint)
                 (lambda (label at)
                   (set! breakpoints 
				   (cons (breakpoint-here label at labels)
                       breakpoints))))
                ((equal? message 'cancel-breakpoint)
                 (lambda (label at)
                   (let ((offset-point (breakpoint-here label at labels)))
                     (if (member offset-point breakpoints)
                         (set! breakpoints 
						 (breakpoint-remove offset-point breakpoints))))))
                 ((equal? message 'cancel-all-breakpoints)
                  (lambda () (set! breakpoints '())))
                 ((equal? message 'proceed-machine)
                  (lambda () (proceed-now)))  
				((equal? message 'save-labels)
                 (lambda (label) (set! labels label)))
...
;;HW8 5.19
(define (extraction-counter count text receive)
	(define (entry-counter label-name insts counter)
		(list label-name insts counter))
	(define (instruction-counter text counter)
		(list text '() counter))
	(cond ((not (null? text))
        (extraction-counter
         (cond ((symbol? (car text)) 
				count)
				(else (+ 1 count)))
		 (cdr text) 
		 (lambda (insts labels) 
             (cond ((symbol? (car text))
                 ;;HW8 5.8 Added
				 (cond ((assoc (car text) labels)
                   (error "the label has existed EXTRACT-LABELS" (car text)))
				 (else (receive insts
                          (cons (entry-counter (car text)
                                                  insts
                                                  count)
                        labels)))))
             (else (receive (cons (instruction-counter (car text) count)
                insts) labels))
	))))
	(else 
	(receive '() '())
...
(define (set-breakpoint machine label at)
  ((machine 'set-breakpoint) label at))
  
(define (proceed-machine machine)
  ((machine 'proceed-machine)))

(define (cancel-breakpoint machine label at)
  ((machine 'cancel-breakpoint) label at))
...
(define (assemble controller-text machine)
  (extract-labels controller-text
                  (lambda (insts labels)
					...
					;;HW8 5.19 Added
					((machine 'save-labels) labels)    
					insts)))				
					
				
;;HW8 5.19
(define (extract-labels text recieve) 
  (extraction-counter 0 text recieve) 
  )
...
;;; make-entry-label creates a pair whose first element is the label
